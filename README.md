...
- **持久媒体生成任务**：Generation Job/Attempt/Event、月度微美元预算预留与 append-only 账本、lease/fencing、运行时版本固定、终态单调转换，以及禁止自动重发的 `submission_unknown` 协调边界。
...
| 后续产品化重点 | 媒体 Worker/回调/poll/人工对账、结果隔离摄取、Job API/SSE、预算管理 UI、数据库字段级加密、真实 Provider 与 S3 压测、开放多租户前的隔离改造 |
...
- `MEDIA_SUBMIT_ENABLED` 仍默认关闭；Job/Attempt、预算、租约和提交不确定态底座已经落地，但真实外部提交仍要等待 Worker 装配、回调/poll 对账、结果隔离摄取和发布故障注入验收完成。
